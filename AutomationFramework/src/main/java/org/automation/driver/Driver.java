	...
	private static void setFirefoxDriver() {
		...
		try {
			if (PropertyUtils.get(ConfigMap.RUNMODE).equalsIgnoreCase(GlobalVars.getGrid())) {
				DriverManager.setDriver(new RemoteWebDriver(new URL(PropertyUtils.get(ConfigMap.REMOTEURL)), cap));
			} else if (PropertyUtils.get(ConfigMap.RUNMODE).equalsIgnoreCase(GlobalVars.getLocal())) {
				if (PropertyUtils.get(ConfigMap.DOWNLOADWEBDRIVER).equalsIgnoreCase(GlobalVars.getYes())) {
	...
	private static void setChromeDriver() {
		...
		try {
			if (PropertyUtils.get(ConfigMap.RUNMODE).equalsIgnoreCase(GlobalVars.getGrid())) {
				...
				// set capabilities if required. Refer website
				DriverManager.setDriver(new RemoteWebDriver(new URL(PropertyUtils.get(ConfigMap.REMOTEURL)), cap));
			} else if (PropertyUtils.get(ConfigMap.RUNMODE).equalsIgnoreCase(GlobalVars.getHeadless())) {
				// set capabilities if required. Refer website
				WebDriverManager.chromedriver().setup();
